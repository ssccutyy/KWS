...
data_kws=/mnt/data/sudi/hi-mia
data_train_root=$data_kws/train
data_dev_root=$data_kws/dev
data_test_root=$data_kws/test

data_train=$data_kws/train
data_test=$data_kws/test
data_dev=$data_kws/dev
mkdir -p $data_train
mkdir -p $data_test
mkdir -p $data_dev
...
if [ $stage -le 0 ];then
    ...
    if [ -f $data_train_root/SPEECHDATA/train.scp ];then
        awk '{print "/mnt/data/sudi/hi-mia/train/SPEECHDATA/"$0 }' $data_train_root/SPEECHDATA/train.scp | grep -E '_02_|_10_' > $data_train_root/SPEECHDATA/train_p.scp
		mv $data_train_root/SPEECHDATA/train_p.scp $data_train_root/wav.scp
    fi
    if [ -f $data_dev_root/SPEECHDATA/dev.scp ];then
        awk '{print "/mnt/data/sudi/hi-mia/dev/SPEECHDATA/"$0 }' $data_dev_root/SPEECHDATA/dev.scp | grep -E '_02_|_10_'  > $data_dev_root/SPEECHDATA/dev_p.scp
        mv $data_dev_root/SPEECHDATA/dev_p.scp $data_dev_root/wav.scp
	fi
    awk '{print "/mnt/data/sudi/hi-mia/test/wav/"$0}' $data_test_root/wav.scp  > $data_test_root/test_p.scp
